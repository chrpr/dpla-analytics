...
recwords = codecs.open('/media/storage/dpla-data/words/records.txt', 'w', encoding='utf-8')
headings = codecs.open('/media/storage/dpla-data/words/headings.txt', 'w', encoding='utf-8')
        ...
        if field in item['sourceResource'] and item['sourceResource'][field] is not None:
            if isinstance(item['sourceResource'][field], list):
                for entry in item['sourceResource'][field]:
                    ...
                    if field in ["creator", "spatial", "subject" ]: 
                        headings.write(ident + "|" + prov + "|" + subprov +"|" + field + "|" + unicode(entry) + "\n")
            ...
            if field in ["creator", "spatial", "subject" ]: 
                headings.write(ident + "|" + prov + "|" + subprov + "|" + field + "|" + unicode(item['sourceResource'][field]) + "\n")
    
    string += " " + prov + " " + subprov + "\n"
    #print string
    #collfiles[cname].write(string + "\n")
    recwords.write(ident + '|"' + string + '"\n')
